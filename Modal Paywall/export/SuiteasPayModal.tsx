...
export interface SuiteasPayModalProps {
  merchant: string;
  /** Starting amount, e.g. 0.12. User can edit down to 0.001 for micropayments. */
  amount?: number;
  merchantLogo?: string;
  markSrc?: string;
  ...
  onPaid?: (result: { txHash: string; amount: number; network: NetworkId; simulated: boolean }) => void;
  onClose?: () => void;
...
function clamp(v: string): number {
  return Math.max(0.001, parseFloat(v) || 0);
}
...
export default function SuiteasPayModal({
  ...
  const amountNum = clamp(amountStr);
  ...
  const handlePay = useCallback(async () => {
    ...
    try {
      ...
      setReceipt(r);
      setStep("paid");
      onPaid?.({ txHash: r.txHash, amount: amountNum, network, simulated: isSepolia });
    } catch (err) {
      setStep("connected");
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", border: `1px solid ${C.n300}`, borderRadius: 10, padding: "6px 10px", gap: 2 }}>
              ...
              <input
                ...
                onBlur={() => setAmountStr(clamp(amountStr).toFixed(2))}
...
/* Usage — real Fuji settlement, no wallet SDK needed:
  <SuiteasPayModal
    ...
    onPaid={({ txHash, amount, network, simulated }) => console.log(txHash, amount, network, simulated)}
  />
